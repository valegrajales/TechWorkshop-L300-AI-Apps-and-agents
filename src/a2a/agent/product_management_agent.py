...
class AgentFrameworkProductManagementAgent:
    ...
    async def invoke(self, user_input: str, session_id: str) -> dict[str, Any]:
        ...
        """
        thread = await self._ensure_thread_exists(session_id)
    ...
    async def _ensure_thread_exists(self, session_id: str) -> AgentThread:
        ...
        if session_id not in self.threads:
            self.threads[session_id] = self.agent.get_new_thread()
