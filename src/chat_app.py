...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def safe_send_text(text: str) -> bool:
        ...
        try:
            ...
            ws_connected = False
            logger.info(f"WebSocket send skipped - connection is closed: {e}")
            return False
        ...
        while True:
            ...
            try:
                ...
                if "WebSocket is not connected" in str(e):
                    ws_connected = False
                    logger.info("WebSocket is not connected; ending session loop")
                    break
                logger.error("Runtime error while receiving WebSocket message", exc_info=True)
                continue
            except Exception:
            ...
            if not user_message and not has_image:
                logger.info("Skipping empty message payload")
                continue
            ...
            try:
                ...
                if not agent_selected or not agent_name:
                    if not await safe_send_text(fast_json_dumps({
            ...
            try:
                ...
                with tracer.start_as_current_span(f"{agent_name.title()} Agent Call"):
                    ...
                    if agent_name == "interior_designer_create_image":
                        ...
                        if not await safe_send_text(response_json):
                            break
                ...
                if agent_name == "cart_manager" and session_loyalty_response and not loyalty_response_sent:
                    ...
                    if not await safe_send_text(fast_json_dumps(loyalty_response_with_cart)):
                        break
    ...
    # =============================================================================
        ...
        try:
            await safe_send_text(fast_json_dumps({
                "answer": "Internal server error",
                "error": str(e),
                "cart": persistent_cart
            }))
